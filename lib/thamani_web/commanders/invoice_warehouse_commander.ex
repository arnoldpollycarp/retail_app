defmodule ThamaniWeb.InvoiceWarehouseCommander do
  use Drab.Commander
  ...
  defhandler fetch_list(socket, sender) do
    ...
    wid = String.to_integer(sender.params["current_user"])

    date = to_string(sender.params["date"])
    date_1 = to_string(sender.params["date_1"])
    list_sales = Sales.list_sales_warehouse_invoice(wid, date, date_1)
    list_sales_uniq = Sales.list_sales_warehouse_invoice_uniq(wid, date, date_1)
    count_sales = Sales.count_sales_warehouse_invoice(wid, date, date_1)
    poke(socket, loader_class: "loader-show")
    sum_sales =
      case Sales.sum_sales_price_warehouse_invoice(wid, date, date_1) do
        nil -> 0
        _ -> Sales.sum_sales_price_warehouse_invoice(wid, date, date_1)
      end
    poke(socket,
  ...
  defhandler generate_invoice(socket, sender) do
    ...
    list_sales = Sales.list_sales_warehouse_invoice(wid, date, date_1)
    list_sales_uniq = Sales.list_sales_warehouse_invoice_uniq(wid, date, date_1)
    value = Sales.sum_sales_price_warehouse_invoice(wid, date, date_1)
    ...
    :file.write_file("uploads/invoice/" <> invoice_number <> ".png", png)
    Path.expand("./uploads/invoice/" <> invoice_number <> ".png")
    logo = Path.expand("./priv/static/images/thamani_logo.png")
    warehouse_name = Dispatches.get_warehouse_name(wid)
    warehouse_owner = Dispatches.get_warehouse_owner(wid)
    warehouse_email = Dispatches.get_warehouse_email(wid)
    ...
    html =
      ...
      <div class='invoice-box'>
       ...
       <tr class='heading'>
             ...
             <td data-field='price' >Unit Price</td>
             <td data-field='retailer' >Total Price</td>
      ...
      <td style='text-align:center'> " <> to_string(Sales.get_quantity_sale_uniq_ware(wid,sale,date,date_1)) <> "  </td>
      <td>"<> to_string(Sales.get_price_sale_uniq_man(sale) )<>"</td>
      <td>"<> to_string(Sales.get_price_sale_uniq_ware(wid,sale,date,date_1)) <>"</td>
...
<tr class='items'>
    ...
    Invoicing.create_invoice_warehouse_node(invoice_number, String.to_integer(items), wid)
