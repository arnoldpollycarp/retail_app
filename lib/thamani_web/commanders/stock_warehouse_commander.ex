defmodule ThamaniWeb.StockWarehouseCommander do
  use Drab.Commander
  ...
  defhandler fetch_stock(socket, sender) do
    ...
    item_id = Inventories.get_breakbulk_id(item)
    quantity_available =
      case Inventories.get_item_count_quantity_stock(item_id, retailer) do
        nil -> 0
        _ -> Decimal.to_integer(Inventories.get_item_count_quantity_stock(item_id, retailer))
      end

    quantity_ware =
          case Inventories.get_item_count_quantity_stock_ware(item_id, retailer) do
            nil -> 0
            _ -> Decimal.to_integer(Inventories.get_item_count_quantity_stock_ware(item_id, retailer))
          end

    quantity_sold =
      case Sales.get_count_quantity_item_stock(retailer,item_id) do
        nil -> 0
        _ -> Decimal.to_integer(Sales.get_count_quantity_item_stock(retailer,item_id))
      end
