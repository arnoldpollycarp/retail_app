defmodule ThamaniWeb.InvoiceRetailCommander do
  use Drab.Commander
  ...
  defhandler fetch_list(socket, sender) do
    ...
    id = String.to_integer(sender.params["current_user"])

    date = to_string(sender.params["date"])
    date_1 = to_string(sender.params["date_1"])
    list_sales = Sales.list_sales_retail_invoice(id, date, date_1)
    list_sales_uniq = Sales.list_sales_retailer_invoice_uniq(id, date, date_1)
    count_sales = Sales.count_sales_retail_invoice(id, date, date_1)
    sum_sales =
      case Sales.sum_sales_price_retail_invoice(id, date, date_1) do
        nil -> 0
        _ -> Sales.sum_sales_price_retail_invoice(id, date, date_1)
      end
  ...
  defhandler generate_invoice(socket, sender) do
    ...
    list_sales = Sales.list_sales_retail_invoice(id, date, date_1)
    list_sales_uniq = Sales.list_sales_retailer_invoice_uniq(id, date, date_1)
    value = Sales.sum_sales_price_retail_invoice(id, date, date_1)
    ...
    warehouse_name = Dispatches.get_warehouse_name(id)
    warehouse_owner = Dispatches.get_warehouse_owner(id)
    warehouse_email = Dispatches.get_warehouse_email(id)
    ...
    html =
      ...
      <td style='text-align:center'> " <> to_string(Sales.get_quantity_sale_uniq_retailer(id,sale,date,date_1)) <> "  </td>
      <td>"<> (:erlang.float_to_binary(Sales.get_price_sale_uniq_retailer(id,sale,date,date_1), decimals: 2)) <>"</td>
...
<tr class='items'>
    ...
    Invoicing.create_invoice_retail_node(invoice_number, String.to_integer(items), id)
